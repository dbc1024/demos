...
@WebService(serviceName = "onlinePayService", // 与接口中指定的name一致
	targetNamespace = "http://webservice.sale.ticket.ectrip.com/", // 与接口中的命名空间一致,一般是接口的包名倒
	endpointInterface = "com.ectrip.ticket.webservice.sale.IOnlinePaymentWebService"// 接口地址
)
/**
 * 在线支付接口，封装环球汇通支付和支付桥支付
 * @author jy
 *
 */
@Component
public class OnlinePaymentWebServiceImpl implements IOnlinePaymentWebService{
...
//            Sysparv5 sys = (Sysparv5) genericDao.get(Sysparv5.class,new Sysparv5Id("HQYT","0001"));
            ...
            res = hqytClient.hqytPayCode(request,null);
            InterfaceLog interfaceLog = new InterfaceLog();
            interfaceLog.setInterfaceType(InterfaceType.PAY);
            interfaceLog.setRequestData(data);
            interfaceLog.setResponseData(JSON.toJSONString(res));
...
//                List list = genericDao.find("from LOrder where id.orid = '" + request.getOrid() + "'");\
                List list = ecService.getLorderByOrid(request.getOrid());
                LOrder lOrder = null;
