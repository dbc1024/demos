...
public class ReportMonthService extends TaskLogService {
	...
	synchronized public int jobrun() {
		...
		System.out.println("�±������п�ʼ:" + Tools.getDayTimes());

		try {
			// �������±���Rprovidermonthtab��
			String today = Tools.getDate(Tools.getDays(), -1);
			List empfznumbList = reportdataDao
					.updateOrQueryempfznumbBymonth(today);
			...
			List empfzList = reportdataDao.updateOrQueryempfzBymonth(today);
			...
			List proList = reportdataDao
					.updateOrQueryProviderByType("2", today);
			...
			if (proList != null && proList.size() >= 1) {
				for (int i = 0; i < proList.size(); i++) {
					...
					Long seq = reportdataDao
							.getMaxPk("id", "Rprovidermonthtab");
					proday.setId(seq + 1);// ����
					proday.setTimes(Tools.getDays());
					proday.setRmonth(Tools.getDays().substring(5, 7));// ��
					proday.setRyear(Tools.getDays().substring(0, 4));// ��
					proday.setIscenicid(Long.parseLong(pro[0].toString()));// �����̱��
			...
			// ������������ϸ����RProviderlisttab��
			List providerList = reportdataDao.updateOrQueryProviderListByType(
					"2", today);
			...
			// �ο���Դ�±���(Rregionmonthtab)
			List sourceList = reportdataDao.updateOrQueryRegionByType("2",
					today);
			...
			List customCountList = reportdataDao
					.updateOrQueryCustomCountByType("2", today);
			...
			// ��ƱԱ��Ʊ�±���(Rsalecounttab)
			List saleCountList = reportdataDao.updateOrQuerySaleCountByType(
					"2", "01", today);// ��Ʊ 2������ 01������Ʊ
			if (saleCountList != null && saleCountList.size() >= 1) {
				for (int i = 0; i < saleCountList.size(); i++) {
					...
					salecount
							.setItickettypeid(Long.parseLong(pro[6].toString()));// ��Ʒ���
			...
			List TuiDingsaleCountList = reportdataDao
					.updateOrQuerySaleCountByType("2", "02", today);// 2������,02������Ʊ
			...
			// �������Ʊ����
			List lxssaleList = reportdataDao.updateOrQueryLxsSaleByDate("2",
					today);
			if (lxssaleList != null && lxssaleList.size() >= 1) {
				for (int i = 0; i < lxssaleList.size(); i++) {
					...
					customgroup.setGrandsonuser(map.get("grandsonuser")
							.toString());// ����Ա
					customgroup.setNumb(Long.parseLong(map.get("numb")
							.toString()));// Ʊ��
					customgroup.setMont(Double.parseDouble(map.get("mont")
							.toString()));// �ܽ��
					customgroup.setIsf(Long
							.parseLong(map.get("isf").toString()));
					customgroup.setDuf(Double.parseDouble(map.get("duf")
							.toString()));
					customgroup.setIsf(Long
							.parseLong(map.get("isf").toString()));// �Ż�����
					customgroup.setIsb(Long
							.parseLong(map.get("isb").toString()));// ��Ʒ���
					customgroup.setIsc(Long
							.parseLong(map.get("isc").toString()));// ������
					customgroup.setIsd(Long
							.parseLong(map.get("isd").toString()));// ��Ⱥ���
					customgroup.setDub(Double.parseDouble(map.get("dub")
							.toString()));// ��Ʒ����
			...
			// ��Ʊ����Rsaletickettab
			List saleticket = reportdataDao.updateOrquerySalebyType("2", today);
			...
			// Ʊ�����ۣ�Rticketmonttab��
			List ticketsalelist = reportdataDao.updateOrQueryTicketSaleByType(
					"2", today);
			...
			List tuantisalecountlist = reportdataDao
					.updateOrQueryCusGroupSaleByType("2", today);
			...
			// �����ͳ���±���
			String todays = today.substring(0, 7).replaceAll("-", "");

			List rwslist = this.reportdataDao.checkRwswarehouse(todays, 2L, 1L);// 2����ͳ�ƣ�
																				// 1��Ԥ��Ʊ
			...
			List houseList = this.reportdataDao
					.showAllHouseware(todays, 2L, 1L);// 2����ͳ�ƣ� 1��Ԥ��Ʊ
			if (houseList != null && houseList.size() > 0) {
				for (int i = 0; i < houseList.size(); i++) {
					...
					List houselist = this.reportdataDao.showIamoutcheckin(
							Long.parseLong(obj[0].toString()), today, 2L, 1L);// 2����ͳ�ƣ�
																				// 1��Ԥ��Ʊ
					// ĳ�ֿ�ĳ�������
			...
			// IC�� �����ͳ���±���
			String kctodays = today.substring(0, 7).replaceAll("-", "");

			List kcrwslist = this.reportdataDao.checkRwswarehouse(kctodays, 2L,
					2L);// 2:��ͳ�ƣ� 2��IC��
			if (kcrwslist != null && kcrwslist.size() > 0) {
				for (int x = 0; x < kcrwslist.size(); x++) {
					Rwswarehousemonth rws = (Rwswarehousemonth) kcrwslist
							.get(x);
					this.reportdataDao.delete(rws);
				}
			}
			List kchouseList = this.reportdataDao.showAllHouseware(kctodays,
					2L, 2L);// 2:��ͳ�ƣ� 2��IC��
			if (kchouseList != null && kchouseList.size() > 0) {
				for (int i = 0; i < kchouseList.size(); i++) {
					...
					// ĳ�ֿ���������
					List houselist = this.reportdataDao.showIamoutcheckin(
							Long.parseLong(obj[0].toString()), today, 2L, 2L);// 2:��ͳ�ƣ�
																				// 2��IC��
					// ĳ�ֿ�ĳ�������
					List warelise = this.reportdataDao.showIamoutcheckOut(
							Long.parseLong(obj[0].toString()), today, 2L, 2L);// 2:��ͳ�ƣ�
		...
		System.out.println("�±������ִ��" + Tools.getDayTimes());
