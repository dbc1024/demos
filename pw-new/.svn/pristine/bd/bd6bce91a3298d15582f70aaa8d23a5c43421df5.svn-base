 ...
 *
 */
public class LineOrderCombin extends OrderCombin{
	...
	public LineOrderCombin(OrderCombinDTO combinDto) {
		super(combinDto);
	}
	...
	protected void combinToOrder(OrderCombinDTO combinDto) {
		...
		while (ite.hasNext()) {  //ѭ��������
			...
			if (provider.getScenictype().equals("07")) {
				...
				if(lprpojo!=null&&lprpojo.getZjlb()!=null&&lprpojo.getZjlb().equals("01")){
					if (lprpojo.getZjhm()!=null) {
						torder.setNotea(lprpojo.getZjhm().substring(6,14));
					}else{
						torder.setNotea(null);
					}
				}else{
					torder.setNotea(null);
				}
...
/*						if(custom.getIbusinessid()==1){
						...
						corwdkindprice = ticketDao.getProductPrice(ticket.getIticketid().toString(), ibusinessid, ticket.getStartdate(), ticket.getCrowdkindid().toString(),
								groupno);
					} catch (Exception e) {
						e.printStackTrace();
					...
					ylist.setId(new YOrderlistId(tlist.getId().getOrderlistid(), morder.getOrid(), provider.getIscenicid()));
			...
			torders.add(torder);
			morder.setMont(morder.getMont() + torder.getMont());
			morder.setZfmont(morder.getZfmont() + torder.getZfmont());
