...
public class CarOrderCombin extends OrderCombin{

	private ITicketDAO ticketDao;

	public CarOrderCombin(OrderCombinDTO combinDto) {
		super(combinDto);
	}
	@Override
	protected void combinToOrder(OrderCombinDTO combinDto) {
		ticketDao = (ITicketDAO) SpringUtil.getBean("ticketDao");
		Custom custom = (Custom) ticketDao.get(Custom.class, combinDto.getUsid());
		String ddzt = "95";//δ���
		/*if (combinDto.getAskOrder() != null && combinDto.getAskOrder().equals("1")) {// ���뵥δ֧������״̬Ϊ24
			ddzt = "24";
		}*/
		 ...
		 * ��������ʼ������
		 */
		getMorder().setOrid(combinDto.getOrid());
		getMorder().setOrtp("01");// Ԥ����
		getMorder().setUsid(custom.getUsid());
		getMorder().setOrda(Tools.getDays());
		getMorder().setOrti(Tools.getNowTime());
		getMorder().setDdzt(ddzt);
		getMorder().setMont(0.0);
		getMorder().setZfmont(0.0);
		getMorder().setTpmont(0.0);
		getMorder().setYhamnt(0.0);
		getMorder().setIsa(0l);
		getMorder().setIsb(0l);
		getMorder().setIsc(0l);
		getMorder().setIsd(0l);
		...
		getMorder().setIsi(0l);
		getMorder().setIsj(0l);
		getMorder().setIsjl(0l);
		getMorder().setYhamnt(0.0);
		getMorder().setTpfs("00");
		getMorder().setIsallcp(0l);
		getMorder().setNote(combinDto.getNote());
		...
		while (ite.hasNext()) {  //ѭ��������
			...
			Object obj = ite.next();
			//�õ�������
			Esbscenicareatab provider = (Esbscenicareatab) ticketDao.get(Esbscenicareatab.class, new Long(obj.toString()));
			//�õ�Ԥ������Ϣ
			for (LprPojo lpr : combinDto.getLprs()) {
				//if (lpr.getIscenicid().equals(obj.toString()) || lpr.getIscenicid().equals(provider.getIparentid().toString())) {
					lprpojo = lpr;
				//}
			}
			//���϶��������̳�Ʊ��ϸ
			...
			//������
			torder.setId(new TOrderId(getMorder().getOrid(), provider.getIscenicid()));
			//��Դ��
			if(lprpojo.getSzregionalid()==null||lprpojo.getSzregionalid().equals("")){
				torder.setIregionalid(1l);  
			}else{
				torder.setIregionalid(new Long(lprpojo.getSzregionalid()));
			}
			torder.setDdzt(getMorder().getDdzt());
			...
			//��ӵ���
			if (lprpojo.getDaoyouid() != null && !lprpojo.getDaoyouid().equals("")) {
				Custom daoyou = (Custom) ticketDao.get(Custom.class, lprpojo.getDaoyouid());
			...
			//��ʼ�������̶���
			yorder.setId(new YOrderId(getMorder().getOrid(), torder.getId().getIscenicid()));
			...
			for (OrderProduct ticket : products) {
				//�õ���Ʊ
				Edmtickettypetab tt = (Edmtickettypetab) ticketDao.get(Edmtickettypetab.class, ticket.getIticketid());
				//�۸�
				Edpcrowdkindtab crowdkind = (Edpcrowdkindtab) ticketDao.get(Edpcrowdkindtab.class, ticket.getCrowdkindid());
				//���ϳ�Ʊ��������Ʊ��ϸ
				List<TZorderlist> tzlists=new ArrayList<TZorderlist>();
				//���϶��������̳�Ʊ��ϸ
				TOrderlist tlist = new TOrderlist();
				if (tt.getIscenicid().intValue() == provider.getIscenicid().intValue()) {
					if (sdate.equals("")) {
					...
					tlist.setDtenddate(Tools.getDate(ticket.getStartdate(), Integer.parseInt(tt.getValidityday().toString()) - 1));
...
/*						if(custom.getIbusinessid()==1){
						...
						corwdkindprice = ticketDao.getProductPrice(ticket.getIticketid().toString(), custom.getIbusinessid().toString(), ticket.getStartdate(), ticket.getCrowdkindid().toString(),
								groupno);
					} catch (Exception e) {
						e.printStackTrace();
					...
					for (int i = 0; i <sontickets.size(); i++) {
						//���Ʊ�۸���
						Edmticketcomposepricetab sonticket = (Edmticketcomposepricetab) sontickets.get(i);
						...
						zorderlist.setDtstartdate(tlist.getDtstartdate() + " " + "00:00:00");
						zorderlist.setDtenddate(tlist.getDtenddate() + " " + "23:59:59");
			...
			getMorder().setMont(getMorder().getMont() + torder.getMont());
			getMorder().setZfmont(getMorder().getZfmont() + torder.getZfmont());
			getMorder().setStdt(sdate);
			getMorder().setTorders(getTorders());
		}
	}
